"""
Script para validar a implementação corrigida do cálculo do IGD com pontos exatos da fronteira de Pareto.
...
do artigo original, utilizando a nova implementação do cálculo do IGD.
"""
...
from src.metrics import calculate_igd_with_exact_pareto
from problems.dtlz import DTLZ1, DTLZ2, DTLZ3, DTLZ4

def validate_corrected_igd():
    """
    Valida a implementação corrigida do cálculo do IGD.
    """
    print("Validando implementação corrigida do cálculo do IGD...")
    ...
    # Executar testes para DTLZ1, DTLZ2, DTLZ3 e DTLZ4 com 3 e 5 objetivos
    results = {}
    ...
    for problem_name in ['DTLZ1', 'DTLZ2', 'DTLZ3', 'DTLZ4']:
        ...
        for n_obj in [3, 5]:
            ...
            # Calcular IGD com a implementação corrigida
            ...
            print(f"IGD corrigido: {igd_value:.6e}")
            ...
            print(f"Razão (IGD/Referência): {igd_value/reference_values[problem_name][n_obj]['median']:.2f}")
            ...
            if n_obj == 3:
                ...
                plt.savefig(os.path.join(output_dir, f"{problem_name}_{n_obj}obj_corrected_igd.png"), dpi=300)
                plt.close()
    
    # Gerar tabela comparativa
    print("\n\nTabela Comparativa de IGD Corrigido:")
    print("=" * 100)
    print(f"{'Problema':<10} {'M':<5} {'IGD (Corrigido)':<20} {'Referência (Mediana)':<20} {'Razão':<10} {'Referência (Best)':<20} {'Referência (Worst)':<20}")
    print("-" * 100)
    ...
    for problem_name in results:
        for n_obj in results[problem_name]:
            ...
            print(f"{problem_name:<10} {n_obj:<5} {results[problem_name][n_obj]['igd']:<20.6e} "
                  ...
                  f"{ratio:<10.2f} "
                  f"{results[problem_name][n_obj]['reference']['best']:<20.6e} "
                  f"{results[problem_name][n_obj]['reference']['worst']:<20.6e}")
    
    print("=" * 100)
    
    # Salvar resultados em arquivo markdown
    with open(os.path.join(output_dir, "corrected_igd_comparison.md"), 'w') as f:
        f.write("# Comparação de Valores de IGD Corrigidos\n\n")
        f.write("## Valores Obtidos vs. Valores do Artigo Original\n\n")
        f.write("| Problema | M | IGD (Corrigido) | Referência (Mediana) | Razão | Referência (Best) | Referência (Worst) |\n")
        f.write("|----------|---|-----------------|----------------------|-------|-------------------|--------------------|\n")
        ...
        for problem_name in results:
            for n_obj in results[problem_name]:
                ...
                f.write(f"| {problem_name} | {n_obj} | {results[problem_name][n_obj]['igd']:.6e} | "
                        ...
                        f"{results[problem_name][n_obj]['reference']['best']:.6e} | "
                        f"{results[problem_name][n_obj]['reference']['worst']:.6e} |\n")
    
    print(f"\nResultados salvos em {os.path.join(output_dir, 'corrected_igd_comparison.md')}")

if __name__ == "__main__":
    validate_corrected_igd()
